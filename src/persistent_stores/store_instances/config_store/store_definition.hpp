...
#endif
{
    ...
    // NTP settings
    StoreItem<bool, false, ItemFlag::network, journal::hash("NTP via DHCP")> ntp_via_dhcp; // use dhcp server for ntp
    StoreItem<std::array<char, 61>, defaults::ntp_server, ItemFlag::network, journal::hash("NTP Address")> ntp_addr; // X.X.X.X address encoded or string
